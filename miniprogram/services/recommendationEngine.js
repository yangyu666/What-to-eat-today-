...
exports.DEFAULT_EXPERIMENT_ID = exports.WEIGHT_PROFILE_ID = exports.ALGORITHM_VERSION = void 0;
exports.recommendRestaurants = recommendRestaurants;
exports.scoreRestaurant = scoreRestaurant;
exports.applyHardFilters = applyHardFilters;
const DEFAULT_LIMIT = 3;
const MIN_PRIMARY_POOL_SIZE = 3;
const MAX_SCORE = 100;
const MIN_SCORE = 0;
exports.ALGORITHM_VERSION = 'recommendation-v2';
exports.WEIGHT_PROFILE_ID = 'breadth-v2';
...
function scoreRestaurant(restaurant, preference, options = {}) {
    ...
    const finalScore = options.fallbackReason !== undefined &&
...
function applyHardFilters(restaurant, preference, excludeRestaurantIds, options) {
    const reasons = [];
    const negativeConflict = getNegativeConflict(restaurant, preference);
    const temperatureConflict = getTemperatureConflict(restaurant, preference);
...
function getAvoidedTagIds(preference) {
    ...
    const wantsMeal = selected.has('intent_meal') ||
...
function getNegativeConflict(restaurant, preference) {
    ...
    tagIds.forEach((tagId) => {
        if (avoided.has(tagId)) {
            tags.add(tagId);
            labels.add(tagId);
            if ((explicitNoSpicy && SPICY_CONFLICT_TAGS.includes(tagId)) ||
...
function getTemperatureConflict(restaurant, preference) {
    ...
    if (wantsHot && hasCold && !hasHot) {
        return { severity: 'soft', label: 'wanted hot food, candidate is cold or light', penalty: 24 };
    }
    if (wantsCold && hasHot && !hasCold) {
        return { severity: 'soft', label: 'wanted cold or light food, candidate is hot-heavy', penalty: 16 };
    }
...
function calculateConfidenceScore(input) {
    let score = input.hardConstraintScore +
